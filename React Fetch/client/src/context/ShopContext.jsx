...
const priceWithDiscount = (p)=> Math.round((Number(p.price) || 0) * (1 - (Number(p.discount) || 0)/100))
...
// normalizador de respuestas de la API
const toArray = (data) => {
  if (Array.isArray(data)) return data
  if (Array.isArray(data?.data)) return data.data      // {data:[...]}
  if (Array.isArray(data?.content)) return data.content // Spring paginado
  ...
  switch(action.type){
    ...
    case 'ADD': {
      ...
      return {
        ...state,
        cart: i
          ? state.cart.map(x => x.id === p.id ? { ...x, qty: (x.qty||1)+1 } : x)
    ...
    case 'SET_QTY':  return { ...state, cart: state.cart.map(i => i.id === action.payload.id ? { ...i, qty: action.payload.qty || 1 } : i) }
...
export function ShopProvider({ children }){
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        const raw = await api.listProducts()          // puede ser [], {data:[]}, {content:[]}, etc.
        ...
        dispatch({ type:'STATUS', payload:{ status:'error', error:String(e?.message||e) } })
      }
    })()
    return () => { alive = false }
  }, [])
  ...
  const filtered = useMemo(() => {
    ...
    return products
      .filter(p => (Number(p?.stock) || 0) > 0)                                   // no mostrar sin stock
      .filter(p => cat === 'all' || String(p?.category||p?.cat||'').toLowerCase() === String(cat).toLowerCase())
  ...
  const total = useMemo(() =>
    state.cart.reduce((t,i) => t + Math.round(Number(i.price||0) * (1 - (Number(i.discount)||0)/100)) * (Number(i.qty)||1), 0)
  , [state.cart])
